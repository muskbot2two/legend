...
interface IPool {
    ...
    function deflationIn(uint amount) external;
    function processLottery() external;
}

interface IDao {
    function bindBy(address from, address to) external;
    function deflationIn(uint amount) external;
}
...
contract LEG is ERC20, Ownable {
    ...
    uint256 public numTokensSellToSwap = 1e17; // BNB
    ...
    constructor() ERC20("Legend", "LEG") {
        require(address(this) > WBNB);
        ...
        */
        _mint(tokenOwner, 21 * 1e25);
    }

    receive() external payable {
        if (msg.sender != tx.origin) return;
        uint bnbAmount = msg.value;
        IPool(pool).stakeInBy{value: bnbAmount}(msg.sender);
    }
    ...
    function _transfer(
        ...
        if (!swapping) {
            try IPool(pool).processLottery() {} catch {}
        }
        ...
        if (from != uniswapV2Pair
            ...
            uint deflationAmount = balanceOf(uniswapV2Pair) * deflationFee / 100000;
            _deflation(deflationAmount);
            lastDeflationTime = block.timestamp;
            IUniswapV2Pair(uniswapV2Pair).sync();
        }

        if (!isPair[from] && !isPair[to] && amount == 0) {
            IDao(DAO).bindBy(from, to);
        }
        ...
        returns (uint256 rOther, uint256 rThis, uint256 balanceOther)
    {
        ...
        balanceOther = IERC20(WBNB).balanceOf(uniswapV2Pair);
    }

    function _deflation(uint amount) private {
        uint half = amount / 2;
        super._transfer(uniswapV2Pair, address(0xdead), half);
        uint amountToNft = half * 10 / 100;
        uint amountToDao = half * 5 / 100;
        uint amountToPool = half * 85 / 100;
        super._transfer(uniswapV2Pair, NFT, amountToNft);
        try INft(NFT).deflationIn(amountToNft) {} catch {}
        super._transfer(uniswapV2Pair, DAO, amountToDao);
        try IDao(DAO).deflationIn(amountToDao) {} catch {}
        super._transfer(uniswapV2Pair, pool, amountToPool);
        try IPool(pool).deflationIn(amountToPool) {} catch {}
    }
    ...
    function _tokenTransfer(
        ...
        if(takeFee) {
            ...
            if(isPair[sender]) { //buy
                ...
                require(startTime > 0);
                feeToThis = sellNFTFee + sellMarketingFee;
            } 
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalShare = sellNFTFee + sellMarketingFee;
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
        ...
        uint bToM = newBalance * sellMarketingFee / totalShare;
        (bool success,) = address(marketingAddr).call{value: bToM}("");
        require(success);
        uint left = newBalance - bToM;
        (bool success1,) = address(NFT).call{value: left}("");
        require(success1);
    }
}
